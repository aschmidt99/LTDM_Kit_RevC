...
void render(LorentzContext *context) {
    ...
    if ((context->frameCount % int(12 - context->sliders[2] * 12) == 0)) {noise0 = float(1-2*random(2));}
    ...
    float noiseFactor0 = noise0 * context->ch[0].noiseScale;
    float fbFactor0 =  -1.0 * context->ch[0].in * context->ch[0].fbGain;

    // apply input + noise to output
    context->ch[0].out =  noiseFactor0 * float(context->buttons[6]) * context->buttons[8] + fbFactor0 * context->buttons[9]; 
